...
master_scale_portfolio <- tbl(con_ews, 'master_scale_portfolio') %>% 
  filter(run_date > eff_date,
         run_date < exp_date) %>% 
  collect()
...
ews_other_portfolio_rating <- ews_other_rating %>% 
  ...
  mutate(grade = case_when(bucket == 'Bucket 5' ~ 'Default',
                           bucket == 'B5: DPD >90' ~ 'Default',
                           bucket == 'B1: MOB <6 & DPD 0-30' ~ 'D0',
                           bucket == 'B3: DPD 31-60' ~ 'D1',
                           bucket == 'B4: DPD 61-90' ~ 'D2',
                           bucket == 'B2: MOB >=6 & DPD 0-30' ~ 'error'
                           ),
