...
def giniNonRetail():
    ...
    _start_date_query = '''
    SELECT MAX(process_date) FROM GINI_Bscore where portfolio in ('Large','Medium', 'Small')
    '''
    ...
    select a.process_date,
            ...
            b.SEGMENT_ODR_LV1 as portfolio,
            a.grade,
            a.grade_num,
            a.score,
            b.default_flag	
    from ews..ews_khdn_score_store a
    ...
    logger.info("Fetching non retail data from the database.")
    df_retail = pd.read_sql_query(non_retail_query, conn_bscore)
    logger.info(f"Non retail data fetched. Number of records: {len(df_retail)}")
